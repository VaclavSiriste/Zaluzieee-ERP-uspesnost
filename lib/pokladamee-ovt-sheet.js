 ...
 *
 * Q (řádek 2 = nadpis, od 3 jména OVT) → target technici
 * P (od řádku 3) → datum zaměření → Splněno targetů ve zvoleném měsíci
 * U (od řádku 3) → kraj → Splněno krajů dle data zaměření
 * K (od řádku 3) → datum navolání (filtr úspěšnosti)
 ...
 * @param {{ startDate: string, endDate: string }} range — filtr úspěšnosti (K) i targetů (P)
 */
export function analyzePokladameeOvtSheet(rows, { startDate, endDate } = {}) {
  ...
  for (let i = startIdx; i < (rows?.length || 0); i += 1) {
    ...
    // Target Splněno: vyplněné Datum zaměření (P) ve zvoleném období
...
async function fetchViaServiceAccount() {
  ...
  // A–U: až Kraj (U); P = datum zaměření, Q = OVT
