...
/**
 * Výčet SLA ze sheetu — vše dle data přijetí leadu (B) ve filtru období:
 * - přišlo / navoláno (K ve stejný den jako B) / chybí / splněno
 * - poptávky + SLA 24/48/72 (K do 24/48/72 h od B)
 */
export function analyzeOvtSheetVycetSla(cfg, rows, { startDate, endDate, todayDate } = {}) {
  ...
  const navolano = periodLeads.filter(
    (item) => item.called && item.navolani_date === item.lead_date
  ).length
...
export function listOvtSheetVycetSlaOrders(
  cfg,
  rows,
  { metric = 'leads', startDate, endDate, limit = 50, offset = 0 } = {}
) {
  const leads = collectOvtSheetSlaLeads(cfg, rows)
  const key = metric === 'fulfilled' ? 'navolano' : metric
  const inPeriod = leads.filter((item) => inRange(item.lead_date, startDate, endDate))

  let filtered = inPeriod
  if (key === 'navolano') {
    filtered = inPeriod.filter((item) => item.called && item.navolani_date === item.lead_date)
  } else if (key === 'missing') {
    filtered = inPeriod.filter((item) => !(item.called && item.navolani_date === item.lead_date))
  } else if (key === 'sla24') {
    filtered = inPeriod.filter((item) => item.sla24 === 1)
  } else if (key === 'sla48') {
    filtered = inPeriod.filter((item) => item.sla48 === 1)
  } else if (key === 'sla72') {
    filtered = inPeriod.filter((item) => item.sla72 === 1)
  }
  // leads | poptavky → celý filtr období
  ...
  return {
    ...
    orders: slice.map((item) => ({
      ...
      called_flag: item.called && item.navolani_date === item.lead_date ? 1 : 0,
      sla24: item.sla24,
      sla48: item.sla48,
      sla72: item.sla72,
