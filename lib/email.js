...
export async function sendLoginEmail({ email, code, magicUrl }) {
  ...
  try {
    ...
    console.warn('[auth] SMTP selhalo, přepínám na dev fallback:', error.message)
    console.log('[auth] Přihlašovací kód pro', email, ':', code)
    console.log('[auth] Magic link:', magicUrl)
    return { delivered: false, devMode: true, smtpError: error.message }
  }
