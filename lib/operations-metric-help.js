...
pokladamee: řádky s důvodem ve sloupci N (mimodosah, duplikace, nemožná realizace, zájem o spolupráci, žádost o práci — bez diakritiky / varianty) se z úspěšnosti úplně vyřazují.`,
...
pokladamee: sloupec U (Kraj) + sloupec P (Datum zaměření) ve zvoleném měsíci.`,
  ...
  filter_obdobi: `Období filtru platí pro SLA příchozích linek (Daktela), Výčet SLA (ERP organization_id), úspěšnost navolání (ERP) i průměrnou dobu do navolání zmeškaných. Vlastní datum = celé kalendářní dny v časové zóně Europe/Prague. Targety používají kalendářní měsíc zvlášť.`,

  vycet_sla_celkem: `Výčet SLA (ERP) = navoláno ve stejný business den / přišlo leadů × 100 %.
Filtr značky: orders.organization_id (company ID) dle listu provozu.
Business den: created_at + 2 h, po 20:00 → další den.
Bez duplikací/reklamací, s ID formuláře.`,

  vycet_sla_leads: `Počet leadů s business datem ve filtru. ERP orders · organization_id značky.`,

  vycet_sla_navolano: `Lead navolán ve stejný business den (first_iframe_change_at::date = business date).`,

  vycet_sla_missing: `Přišlo leadů minus dnes navoláno (business den).`,

  vycet_sla_poptavky: `Poptávky dle kalendářního data (created_at + 2 h). Stejné base filtry + u ostatních značek vyloučení proc_nedopadl = Venkovky (u org 8 ne).`,

  vycet_sla_24: `SLA 24: kontakt do 24 h od vzniku leadu (oba časy +2 h). % = SLA 24 / poptávky.`,
