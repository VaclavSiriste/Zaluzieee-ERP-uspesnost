...
import {
  buildIsWorkingHoursSql,
  resolveWorkingHoursProfile,
  WORKING_HOURS_RULE
} from '@/lib/working-hours-sql'

export { WORKING_HOURS_RULE }
...
export function buildIncomingLineSummarySql({ brandId = 'cz' } = {}) {
  const response = RESPONSE_SECONDS_SQL.trim()
  const slaQueueFilter = buildSlaIncomingQueueFilterSql(brandId)
  const brandLinesFilter = buildSlaQueueFilterSql(brandId)
  const isWorking = buildIsWorkingHoursSql('c.call_time', {
    brandId: resolveBrandWorkingHoursProfile(brandId)
  })

  // Širší WHERE = všechny linky značky (pracovní doba).
  // SLA metriky filtrují jen fronty countsForSla uvnitř FILTER.
  const slaPred = slaQueueFilter.replace(/^\s*AND\s+/i, '').trim() || 'TRUE'
  const brandPred = brandLinesFilter.replace(/^\s*AND\s+/i, '').trim() || 'TRUE'

  return `
    SELECT
      COUNT(*) FILTER (WHERE ${slaPred})::int AS total_incoming,
      COUNT(*) FILTER (WHERE c.answered IS TRUE AND (${slaPred}))::int AS answered,
      COUNT(*) FILTER (
        WHERE COALESCE(c.answered, false) IS NOT TRUE AND (${slaPred})
      )::int AS missed,
      COUNT(*) FILTER (
      ...
      COUNT(*) FILTER (
        WHERE c.answered IS TRUE
          AND COALESCE(${response}, 0) > ${SLA_THRESHOLD_SECONDS}
          AND (${slaPred})
      )::int AS over_20s,
      COUNT(*) FILTER (
        WHERE c.answered IS TRUE
      ...
      COUNT(*) FILTER (
        WHERE c.answered IS TRUE
          AND COALESCE(${response}, 0) BETWEEN 21 AND 40
          AND (${slaPred})
      )::int AS interval_21_40,
      COUNT(*) FILTER (
        WHERE c.answered IS TRUE
          AND COALESCE(${response}, 0) BETWEEN 41 AND 60
          AND (${slaPred})
      )::int AS interval_41_60,
      COUNT(*) FILTER (
        WHERE c.answered IS TRUE
          AND COALESCE(${response}, 0) > 60
          AND (${slaPred})
      )::int AS interval_60_plus,
      AVG(${response}) FILTER (
        WHERE c.answered IS TRUE AND (${slaPred})
      )::float8 AS avg_response_seconds,

      COUNT(*) FILTER (
        WHERE (${isWorking}) IS TRUE AND (${brandPred})
      )::int AS working_hours_calls,
      COUNT(*) FILTER (
        WHERE (${isWorking}) IS FALSE AND (${brandPred})
      )::int AS outside_hours_calls,
      COUNT(*) FILTER (
        WHERE (${isWorking}) IS TRUE
          AND COALESCE(c.answered, false) IS NOT TRUE
          AND (${brandPred})
      )::int AS working_hours_missed,
      COUNT(*) FILTER (
        WHERE (${isWorking}) IS FALSE
          AND COALESCE(c.answered, false) IS NOT TRUE
          AND (${brandPred})
      )::int AS outside_hours_missed
    ${INCOMING_CALLS_FROM}
...
export function buildQueueBreakdownSql(segments = [], { brandId = 'cz' } = {}) {
...
export function normalizeQueueBreakdown(rows = [], segments = []) {
  ...
  const items = segments
    .map((segment) => {
      ...
      return {
        ...
        working_hours_calls: Number(row.working_hours_calls) || 0,
        outside_hours_calls: Number(row.outside_hours_calls) || 0,
        working_hours_missed: Number(row.working_hours_missed) || 0,
        outside_hours_missed: Number(row.outside_hours_missed) || 0
      }
    })
  ...
  const totals = items.reduce(
    (acc, item) => ({
      ...
      working_hours_calls: acc.working_hours_calls + item.working_hours_calls,
      outside_hours_calls: acc.outside_hours_calls + item.outside_hours_calls,
      working_hours_missed: acc.working_hours_missed + item.working_hours_missed,
      outside_hours_missed: acc.outside_hours_missed + item.outside_hours_missed
    }),
    {
      total_calls: 0,
      answered: 0,
      ...
      outside_hours_calls: 0,
      working_hours_missed: 0,
      outside_hours_missed: 0
    }
  )
...
export async function fetchQueueBreakdown({ brandId, startDate, endDate }) {
  ...
  const sql = buildQueueBreakdownSql(segments, { brandId })
  if (!sql) return null
...
export function normalizeIncomingLineSummary(row = {}, { brandId = 'cz' } = {}) {
  ...
  return {
    ...
    working_hours_missed: Number(row.working_hours_missed) || 0,
    outside_hours_missed: Number(row.outside_hours_missed) || 0,
    working_hours_rule: hoursProfile.label,
