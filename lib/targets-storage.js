...
import {
  buildDefaultSlovakRegionCatalog,
  resolveSlovakErpRegionId
...
const BRAND_KEY = 'prvni.targets.brand'
...
export function resolveRegionCatalogForBrand(brandId = 'cz') {
  const id = resolveTargetsBrandId(brandId)
  if (id === 'sk') return buildDefaultSlovakRegionCatalog()
  return buildDefaultRegionCatalog()
}

export function resolveErpRegionIdForBrand(erpRegion, brandId = 'cz', catalog) {
  const id = resolveTargetsBrandId(brandId)
  const cat = catalog || resolveRegionCatalogForBrand(id)
  if (id === 'sk') return resolveSlovakErpRegionId(erpRegion, cat)
  return resolveErpRegionId(erpRegion, cat)
}
...
function emptyMonthBucket(brandId = 'cz') {
  const id = resolveTargetsBrandId(brandId)
  const useSheetTechs = id === 'pokladamee' || id === 'malujemeee'
  const techCatalog = useSheetTechs ? [] : buildDefaultCatalog()
  const regionCatalog = resolveRegionCatalogForBrand(id)
  return {
    technicians: {
      catalog: techCatalog,
      activeIds: techCatalog.map((item) => item.id),
      values: {},
      completed: {}
    },
    regions: {
      catalog: regionCatalog,
      activeIds: regionCatalog.map((item) => item.id),
      values: {},
      completed: {},
      labels: {}
    },
...
function remapRegionKeyedMaps(rawRegions, catalog, brandId = 'cz') {
  function remap(map) {
    const next = {}
    ...
    for (const [key, value] of Object.entries(map)) {
      ...
      const canonicalId =
        resolveErpRegionIdForBrand(label, brandId, catalog) ||
        resolveErpRegionIdForBrand(key, brandId, catalog) ||
...
function remapRegionActiveIds(rawActiveIds, rawRegions, catalog, brandId = 'cz') {
  ...
  if (resolveTargetsBrandId(brandId) !== 'sk') {
...
function normalizeMonthBucket(raw, brandId = 'cz') {
  const bucket = emptyMonthBucket(brandId)
  ...
  // OVT sheet značky: prázdný katalog dokud nenačteme sheet (ne fallback na ERP jména)
  const sheetBrand =
    resolveTargetsBrandId(brandId) === 'pokladamee' ||
    resolveTargetsBrandId(brandId) === 'malujemeee'
  if (sheetBrand) {
    bucket.technicians.catalog = sortTechnicians(techCatalog)
  } else {
    bucket.technicians.catalog = techCatalog.length
      ? sortTechnicians(techCatalog)
      : bucket.technicians.catalog
  }
  ...
  const remappedRegions = remapRegionKeyedMaps(raw.regions, regionCatalog, brandId)
  ...
  bucket.regions.activeIds = remapRegionActiveIds(
    raw.regions?.activeIds,
    raw.regions,
    regionCatalog,
    brandId
  )
...
function migrateLegacyTechnicians(monthKey, store, brandId = 'cz') {
  ...
  if (!store[monthKey]) store[monthKey] = emptyMonthBucket(brandId)
...
export function readTargetsBrandId() {
  const raw = readJson(BRAND_KEY, 'cz')
  const id = resolveTargetsBrandId(raw)
  return id === 'sk' ? 'sk' : 'cz'
}

export function writeTargetsBrandId(brandId) {
  writeJson(BRAND_KEY, resolveTargetsBrandId(brandId) === 'sk' ? 'sk' : 'cz')
}
...
export function readMonthBucket(monthKey, brandId = 'cz') {
  ...
  if (!store[key]) {
    store[key] = emptyMonthBucket(brandId)
    writeMonthlyStore(brandId, store)
  }
  return normalizeMonthBucket(store[key], brandId)
}

export function writeMonthBucket(monthKey, bucket, brandId = 'cz') {
  const store = readMonthlyStore(brandId)
  store[monthKey] = normalizeMonthBucket(bucket, brandId)
  writeMonthlyStore(brandId, store)
}
