...
/**
 * Filtry poptávky / SLA 24·48·72:
 * - stav: vyloučit duplikace + obsahuje reklamace
 * - ID formuláře: vyloučit null
 * - všechny kraje včetně Karlovarského
 * - proc_nedopadl_hovor: Vyloučit Rovno Venkovky OR Zahrnout Je nula
 *   (u značky Venkovky / organization_id 8 se tento filtr nepoužije)
 */
export const SLA_POPTAVKY_VENKOVKY_EXCLUDE_SQL = `
  AND NOT EXISTS (
    SELECT 1
...
export const SLA_POPTAVKY_FILTERS_SQL = `
  ...
  ${SLA_POPTAVKY_VENKOVKY_EXCLUDE_SQL}
`

export function buildSlaPoptavkyFiltersSql({ excludeVenkovkyReason = true } = {}) {
  if (!excludeVenkovkyReason) return SLA_BASE_FILTERS_SQL
  return SLA_POPTAVKY_FILTERS_SQL
}

/**
 * Filtr značky v ERP: orders.organization_id (uživatelé často říkají „company ID“).
 * @returns {{ sql: string, params: any[] }}
 */
export function appendOrganizationFilter(params, organizationId) {
  const next = Array.isArray(params) ? [...params] : []
  ...
  next.push(parsed)
  return {
    sql: `AND o.organization_id = $${next.length}`,
    params: next
  }
}

/** U Venkovek (org 8) nefiltrujeme „proc_nedopadl = venkovky“. */
export function shouldExcludeVenkovkyReason(organizationId, brandId) {
  if (String(brandId || '').toLowerCase() === 'venkovky') return false
  if (organizationId != null && Number(organizationId) === 8) return false
  return true
}
