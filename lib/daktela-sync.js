...
async function triggerGithubSync() {
  ...
  if (!token) {
    throw new Error(
      'Na produkci nejde spustit lokální sync. Přidej do Vercel env GITHUB_SYNC_TOKEN (repo token s právem actions:write pro VaclavSiriste/pohoda), nebo spusť sync lokálně / ručně v GitHub Actions.'
    )
  }
...
export async function getDaktelaSyncStatus() {
  ...
  let githubRun = null
  // Na Vercelu status soubor nemusí přežít mezi requesty → ber stav z GitHub Actions
  if (status.mode === 'github' || process.env.VERCEL || process.env.GITHUB_SYNC_TOKEN) {
    ...
    if (githubRun) {
      ...
      if (githubRun.status === 'in_progress' || githubRun.status === 'queued') {
        status = {
          ...
          state: 'running',
          mode: 'github',
          message: 'Sync běží na GitHub Actions…',
          github: githubMeta,
          percent: status.percent || 40
        }
  ...
  return {
    ...status,
    progress: buildProgress(status, live),
    live,
    logTail,
    dataFreshness,
    canLocal,
