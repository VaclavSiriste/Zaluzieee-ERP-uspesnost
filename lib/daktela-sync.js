...
import {
  SCRIPT_LABELS,
  formatScriptsLabel,
  normalizeSyncScripts,
  resolveSyncScopeForPath
} from '@/lib/daktela-sync-scope'
...
function resolveScriptList(options = {}) {
  if (Array.isArray(options.scripts) && options.scripts.length) {
    return normalizeSyncScripts(options.scripts)
  }
  if (options.pathname) {
    const scope = resolveSyncScopeForPath(options.pathname)
    if (scope.needsDaktela) return scope.scripts
  }
  return normalizeSyncScripts(DASHBOARD_SCRIPTS)
}

function estimateMinutesForScripts(scripts) {
  if (!scripts?.length) return 0
  const base = Number(process.env.GITHUB_SYNC_ESTIMATE_MINUTES) || 12
  // hrubý odhad: plný set ~12 min, lineárně podle počtu tabulek (min 3)
  const fullCount = String(DASHBOARD_SCRIPTS).split(',').filter(Boolean).length || 7
  return Math.max(3, Math.round((base * scripts.length) / fullCount))
}

function buildProgress(status, live) {
  const scripts =
    Array.isArray(status.syncScripts) && status.syncScripts.length
      ? status.syncScripts
      : String(DASHBOARD_SCRIPTS)
          .split(',')
          .map((s) => s.trim())
          .filter(Boolean)
  const steps =
  ...
  return {
    ...
    steps,
    syncScripts: scripts,
    scopeLabel: status.scopeLabel || null
  }
}
...
async function triggerRailwaySync(options = {}) {
  ...
  const scopeLabel = options.scopeLabel || formatScriptsLabel(scriptList)
  writeStatusFile({
    state: 'running',
    mode: 'railway',
    ...
    syncScripts: scriptList,
    scopeLabel,
    estimateMinutes: estimateMinutesForScripts(scriptList),
    message:
      body.message ||
      `Sync spuštěn na Railway (${scopeLabel}).`,
  ...
  return {
    ...
    message:
      body.message ||
      `Synchronizace běží na Railway (${scopeLabel}).`
  }
}
...
async function triggerGithubSync(options = {}) {
  ...
  const scriptList = resolveScriptList(options)
  const scopeLabel = options.scopeLabel || formatScriptsLabel(scriptList)
  const estimateMinutes = options.estimateMinutes || estimateMinutesForScripts(scriptList)
  ...
  const response = await fetch(
    `https://api.github.com/repos/${owner}/${repoName}/actions/workflows/${workflow}/dispatches`,
    {
      ...
      body: JSON.stringify({
        ref,
        inputs: {
          sync_scripts: scriptList.join(',')
        }
      })
    }
  )

  // Workflow na GitHubu ještě nemá input sync_scripts → fallback na plný sync
  if (response.status === 422) {
    const errBody = await response.text()
    if (/Unexpected inputs|sync_scripts/i.test(errBody)) {
      ...
      if (!fallback.ok) {
        ...
        throw new Error(`GitHub Actions (${fallback.status}): ${body.slice(0, 300)}`)
      }
      const startedAt = new Date().toISOString()
      writeStatusFile({
        state: 'running',
        ...
        syncScripts: scriptList,
        scopeLabel,
        estimateMinutes: Number(process.env.GITHUB_SYNC_ESTIMATE_MINUTES) || 12,
        message:
          `Sync spuštěn na GitHub Actions (plný set — pushni pohoda workflow se sync_scripts pro užší sync stránky).`,
        github: { repo, workflow, ref, scripts: scriptList, scopedInput: false }
      })
      return {
        started: true,
        ...
        scopeLabel,
        message:
          'Synchronizace běží na GitHubu (zatím plný set). Po pushi workflow v repo pohoda půjde jen rozsah stránky.'
      }
    }
    throw new Error(`GitHub Actions (422): ${errBody.slice(0, 300)}`)
  }
  ...
  writeStatusFile({
    ...
    scopeLabel,
    estimateMinutes,
    message: `Sync spuštěn na GitHub Actions (${scopeLabel}, odhad ~${estimateMinutes} min).`,
    github: { repo, workflow, ref, scripts: scriptList }
  })
  ...
  return {
    ...
    scopeLabel,
    estimateMinutes,
    message: `Synchronizace běží na GitHubu — ${scopeLabel} (odhad ~${estimateMinutes} min).`
  }
}

function startLocalSync(options = {}) {
  ...
  writeStatusFile({
    ...
    message: `Spouštím stahování z Daktely (${scopeLabel})…`
  })

  try {
    const logFd = fs.openSync(LOG_FILE, 'a')
    fs.writeSync(logFd, `\n--- API sync trigger ${startedAt} scripts=${scriptList.join(',')} ---\n`)
    fs.closeSync(logFd)
  } catch {
  ...
  try {
    const child = spawn(process.execPath, [RUNNER_SCRIPT], {
      ...
      env: {
        ...process.env,
        SYNC_GHA_SCRIPTS: scriptList.join(',')
      },
      detached: true,
      stdio: 'ignore'
    })
    ...
    return {
      ...
      scripts: scriptList,
      scopeLabel,
      message: `Synchronizace běží na pozadí (${scopeLabel}).`
    }
  } catch (err) {
    writeStatusFile({
      state: 'error',
...
export async function startDaktelaSync(options = {}) {
  const pathname = options.pathname || ''
  const scope = pathname
    ? resolveSyncScopeForPath(pathname)
    : {
        scripts: resolveScriptList(options),
        label: options.scopeLabel || null,
        needsDaktela: true,
        estimateMinutes: estimateMinutesForScripts(resolveScriptList(options))
      }

  const scriptList =
    Array.isArray(options.scripts) && options.scripts.length
      ? normalizeSyncScripts(options.scripts)
      : scope.scripts
  ...
  if (!scriptList.length) {
    return {
      ...
      message:
        scope.label ||
        'Na této stránce Daktela sync není potřeba (data jdou z ERP). Obnov stránku / filtr.'
    }
  }
  ...
  const opts = {
    ...options,
    scripts: scriptList,
    scopeLabel: options.scopeLabel || scope.label || formatScriptsLabel(scriptList),
    estimateMinutes: options.estimateMinutes || scope.estimateMinutes || estimateMinutesForScripts(scriptList)
  }

  if (canRunLocalSync()) {
    return startLocalSync(opts)
  }
  if (getRailwaySyncConfig()) {
    return triggerRailwaySync(opts)
  }
  return triggerGithubSync(opts)
}
...
function buildGithubEstimate(status, githubRun) {
  const estimateMinutes = Math.max(
    3,
    Number(status.estimateMinutes) ||
      Number(process.env.GITHUB_SYNC_ESTIMATE_MINUTES) ||
      12
  )
