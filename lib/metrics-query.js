...
/** Od 1. dne aktuálního měsíce do dnes (lokální kalendář). */
export function getMonthToDateRange(now = new Date()) {
...
export function resolveDateRange({ startDate, endDate, period }) {
  ...
  const endIsDateOnly = Boolean(endDate && /^\d{4}-\d{2}-\d{2}$/.test(String(endDate).trim()))
  const startIsDateOnly = Boolean(startDate && /^\d{4}-\d{2}-\d{2}$/.test(String(startDate).trim()))

  let end = endDate ? parseDay(endDate) : now
  let start = startDate ? parseDay(startDate) : null
  ...
  return {
    // Vlastní datum z UI = celý kalendářní den; relativní období začíná od 00:00.
    start: startIsDateOnly || !startDate ? startOfLocalDay(start) : start,
    end: endIsDateOnly ? endOfLocalDay(end) : end
  }
}

/** Kalendářní rozsah pro Daktela (Europe/Prague) — stejný filtr jako ve FilterAssistant. */
export function resolveDaktelaDateRange({ startDate, endDate, period }) {
...
/** Lokální YYYY-MM-DD pro ERP filtry podle data (bez UTC posunu). */
