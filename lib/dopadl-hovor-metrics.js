  ...
  const operators = [...operatorKeys]
    .map((operatorKey) => {
      ...
      return {
        ...
        dopadl_hovor_ne: Number(dopadl?.dopadl_hovor_ne) || 0,
        dopadl_hovor_pocet: Number(dopadl?.dopadl_hovor_pocet) || 0,
        ...
        success_navolani_pct: dopadlAno > 0 ? (zamereniAno / dopadlAno) * 100 : null
      }
    })
    .filter((row) => row.dopadl_hovor_ano > 0 || row.domluveno_zamereni_ano > 0)
