...
import { getDateFilterSql, SYSTEEEM_ORDER_URL } from '@/lib/metrics-query'
...
const METRIC_DEFS = {
  ...
  domluveno_zamereni: {
    ...
    labelAno: 'Domluveno zaměření ANO',
    labelPocet: 'Domluveno zaměření (Počet)'
  }
}
...
async function fetchErpHovoryByOperator({ start, end }) {
  ...
  const { rows } = await pool.query(
    ...
    ORDER BY 1
    `,
    [start, end, ERP_HOVORY_EXCLUDED_OPERATORS.map((n) => n.toLowerCase())]
  )
...
async function fetchErpHovoryOrders({
  ...
  const { dateFilterCte, dateFilterJoin, dateFilterWhere } = getDateFilterSql('navolani')
  const params = [start, end, ERP_HOVORY_EXCLUDED_OPERATORS.map((n) => n.toLowerCase())]
  let paramIndex = 4
...
async function fetchYesNoByOperator(def, { start, end }) {
  ...
  const { rows } = await pool.query(
    ...
    SELECT
      COALESCE(NULLIF(TRIM(u.name), ''), 'Nepřiřazený operátor') AS operator_name,
      COUNT(*) FILTER (WHERE mv.raw_value IS NOT NULL)::int AS pocet,
    ...
    ORDER BY 1
    `,
    [start, end, def.valueSlug, def.assignmentType]
  )

  return rows.map((row) => ({
    operator_name: row.operator_name,
    operator_key: normalizeOperatorKey(row.operator_name),
    [def.pocetKey]: Number(row.pocet) || 0,
    [def.anoKey]: Number(row.ano) || 0
  }))
}
...
async function fetchYesNoOrders(def, { start, end, operatorName, mode = 'ano', limit = 50, offset = 0 }) {
  ...
  const { dateFilterCte, dateFilterJoin, dateFilterWhere } = getDateFilterSql('navolani')
  const params = [start, end, def.valueSlug, def.assignmentType]
  let paramIndex = 5
  const operatorFilter = operatorName
    ? `AND LOWER(TRIM(COALESCE(u.name, ''))) = LOWER(TRIM($${paramIndex++}))`
    : ''
  ...
  const valueFilter =
    mode === 'ano' ? `AND mv.value_lc = 'ano'` : `AND mv.raw_value IS NOT NULL`
...
export { fetchErpHovoryByOperator, fetchErpHovoryOrders }
...
export async function fetchErpYesNoOrders({ metric, ...args }) {
  ...
  if (key === 'erp_hovory') {
    ...
    return {
      ...result,
      label: mode === 'ano' ? 'ERP hovory ANO' : 'ERP hovory (Ano + Ne)',
      valueAlias: 'dopadl_hovor'
    }
  }
