...
export function resolveErpMetricKey(metric) {
  if (metric === 'dopadl_hovor_ano' || metric === 'dopadl_hovor_ne' || metric === 'dopadl_hovor_pocet') {
    return 'dopadl_hovor'
  }
  if (metric === 'domluveno_zamereni_ano' || metric === 'domluveno_zamereni_ne' || metric === 'domluveno_zamereni_pocet') {
...
async function fetchYesNoByOperator(def, { start, end, organizationId = null }) {
  const pool = getPool()
  if (!pool) return []

  const { dateFilterCte, dateFilterJoin, dateFilterWhere } = getDateFilterSql('navolani')
  const baseParams = [formatDateOnly(start), formatDateOnly(end), def.valueSlug, def.assignmentType]
  const org = appendOrganizationFilter(baseParams, organizationId)
...
async function fetchYesNoOrders(def, { start, end, operatorName, mode = 'ano', limit = 50, offset = 0, organizationId = null }) {
  const pool = getPool()
  if (!pool) throw new Error('Chybí ERP_DB_CONNECTION_STRING')
...
export async function fetchDopadlHovorSummary({ start, end, organizationId = null }) {
  const [dopadlRows, domluvenoRows] = await Promise.all([
    fetchDopadlHovorByOperator({ start, end, organizationId }),
    fetchDomluvenoZamereniByOperator({ start, end, organizationId })
  ])

  const domluvenoByKey = new Map(domluvenoRows.map((row) => [row.operator_key, row]))
  const dopadlByKey = new Map(dopadlRows.map((row) => [row.operator_key, row]))

  let dopadl_hovor_ano = 0
  let dopadl_hovor_ne = 0
  let dopadl_hovor_pocet = 0
  let domluveno_zamereni_ano = 0
  let domluveno_zamereni_ne = 0
  let domluveno_zamereni_pocet = 0

  for (const row of dopadlRows) {
    dopadl_hovor_ano += Number(row.dopadl_hovor_ano) || 0
    dopadl_hovor_ne += Number(row.dopadl_hovor_ne) || 0
    dopadl_hovor_pocet += Number(row.dopadl_hovor_pocet) || 0
  }

  for (const row of domluvenoRows) {
    domluveno_zamereni_ano += Number(row.domluveno_zamereni_ano) || 0
    domluveno_zamereni_ne += Number(row.domluveno_zamereni_ne) || 0
    domluveno_zamereni_pocet += Number(row.domluveno_zamereni_pocet) || 0
  }

  const operatorKeys = new Set([...dopadlByKey.keys(), ...domluvenoByKey.keys()])
  const operators = [...operatorKeys]
    .map((operatorKey) => {
      const dopadl = dopadlByKey.get(operatorKey)
      const domluveno = domluvenoByKey.get(operatorKey)
      const dopadlAno = Number(dopadl?.dopadl_hovor_ano) || 0
      const dopadlNe = Number(dopadl?.dopadl_hovor_ne) || 0
      const dopadlPocet = Number(dopadl?.dopadl_hovor_pocet) || dopadlAno + dopadlNe
      const zamereniAno = Number(domluveno?.domluveno_zamereni_ano) || 0
      return {
        operator_name: dopadl?.operator_name || domluveno?.operator_name || operatorKey,
        dopadl_hovor_ano: dopadlAno,
        dopadl_hovor_ne: dopadlNe,
        dopadl_hovor_pocet: dopadlPocet,
        domluveno_zamereni_ano: zamereniAno,
        domluveno_zamereni_ne: Number(domluveno?.domluveno_zamereni_ne) || 0,
        domluveno_zamereni_pocet: Number(domluveno?.domluveno_zamereni_pocet) || 0,
        // Úspěšnost navolání = Dopadl hovor ANO / (ANO + NE)
        success_navolani_pct: dopadlPocet > 0 ? (dopadlAno / dopadlPocet) * 100 : null,
        // Úspěšnost trasování = Naplánován termín zaměření ANO / Dopadl hovor ANO
        success_trasovani_pct: dopadlAno > 0 ? (zamereniAno / dopadlAno) * 100 : null
      }
    })
    .filter((row) => row.dopadl_hovor_pocet > 0 || row.domluveno_zamereni_ano > 0)
    .sort((a, b) => b.dopadl_hovor_ano - a.dopadl_hovor_ano)
  ...
  return {
    ...
    domluveno_zamereni_ne,
    domluveno_zamereni_pocet,
    organization_id: organizationId != null ? Number(organizationId) : null,
    // Úspěšnost navolání = Dopadl hovor ANO / (ANO + NE)
    success_navolani_pct:
      dopadl_hovor_pocet > 0 ? (dopadl_hovor_ano / dopadl_hovor_pocet) * 100 : null,
    // Úspěšnost trasování = Naplánován termín zaměření ANO / Dopadl hovor ANO
    success_trasovani_pct:
      dopadl_hovor_ano > 0 ? (domluveno_zamereni_ano / dopadl_hovor_ano) * 100 : null,
...
export async function fetchErpYesNoOrders({ metric, ...args }) {
  ...
  const mode = metricKey.endsWith('_pocet') ? 'pocet' : metricKey.endsWith('_ne') ? 'ne' : 'ano'
  ...
  return {
    ...result,
    label:
      mode === 'ano' ? def.labelAno : mode === 'ne' ? `${def.labelAno.replace(' ANO', ' NE')}` : def.labelPocet,
