...
export const OPERATIONS_BRANDS = {
  cz: {
    ...
    showTargets: true,
    showTrasovacMetrics: true,
    showTrasovaniSuccess: true,
    targetsBrandId: 'cz',
    navolaniHint: 'ERP · organizace č. 5 (zaluzieee) · Dopadl hovor ANO / (ANO + NE)',
    trasovaniHint:
      'ERP · organizace č. 5 (zaluzieee) · Naplánován termín zaměření ANO / Dopadl hovor ANO',
  ...
  sk: {
    ...
    showTargets: true,
    targetsBrandId: 'sk',
    navolaniHint: 'ERP · organizace SK · Dopadl hovor ANO / (ANO + NE)',
  ...
  malujemeee: {
    ...
    /** Úspěšnost + jména techniků + targety z Google Sheet OVT */
    navolaniSource: 'ovt-sheet',
    vycetSlaSource: 'ovt-sheet',
    navolaniHint:
      'OVT sheet · Dopadl hovor ANO / (ANO + NE) · Q technici · P datum zaměření = Splněno targetů',
  ...
  pokladamee: {
    ...
    /** Úspěšnost + jména techniků + targety z Google Sheet OVT (Databáze_2026) */
    navolaniSource: 'ovt-sheet',
    navolaniHint:
      'OVT sheet · Dopadl hovor ANO / (ANO + NE) · bez M: mimodosah, duplikace, nemožná realizace, zájem o spolupráci, žádost o práci',
    /** Pracovní doba jen Po–Pá 8–16 (víkendy = mimo) */
    workingHoursProfile: 'weekday_8_16',
  ...
  venkovky: {
    ...
    navolaniHint: 'ERP · organizace č. 8 (Venkovky) · Dopadl hovor ANO / (ANO + NE)',
...
export function resolveBrandWorkingHoursProfile(brandId) {
  const brand = resolveOperationsBrand(brandId)
  if (brand?.workingHoursProfile) return brand.workingHoursProfile
  if (String(brandId || '').toLowerCase() === 'pokladamee') return 'weekday_8_16'
  return 'default'
}
