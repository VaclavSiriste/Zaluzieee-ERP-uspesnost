/**
 * Malujemeee OVT — list podle gid z URL (nemění se při přejmenování)
 * https://docs.google.com/spreadsheets/d/1_VBk_iUklmFN7JtK7PPpBp0vtd3K1Lor4oowMurTXvM/edit?gid=0
 *
 * Nasazení:
 * 1) Otevři sheet → Rozšíření → Apps Script → vlož tento soubor (Code.gs)
 * 2) Menu „Malujemeee OVT API“ → Nastavit API token
 *    NEBO PropertiesService: SHEET_API_TOKEN = malujemeee-ovt-2026
 * 3) Nasadit → Nové nasazení → Typ: Webová aplikace
 *    - Spouštět jako: Já
 *    - Kdo má přístup: Kdokoli (nebo Kdokoli s Google účtem)
 * 4) URL nasazení dej do Prvni/.env jako MALUJEMEEE_OVT_SHEET_WEBAPP_URL
 *    a token jako MALUJEMEEE_OVT_SHEET_TOKEN=malujemeee-ovt-2026
 *
 * Sloupce (stejně jako pokladamee): K = datum navolání, L = dopadl hovor,
 *   P = datum zaměření, Q = OVT technik, U = kraj
 * Řádek 2 = nadpis, data od řádku 3.
 */

var SPREADSHEET_ID = '1_VBk_iUklmFN7JtK7PPpBp0vtd3K1Lor4oowMurTXvM';
/** Stabilní ID listu z URL (?gid=0) */
var SHEET_GID = 0;
...
var COL_KRAJ = 21; // U
var SOURCE_LABEL = 'malujemeee-ovt-sheet';
...
function setSheetApiToken() {
  ...
  var result = ui.prompt(
    'API token pro Prvni dashboard (malujemeee)',
    'Zadej tajný token (doporučeno: malujemeee-ovt-2026):',
