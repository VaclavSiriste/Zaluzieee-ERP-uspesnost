/**
 * Export zakázek → Google Sheet, list testdata2
 * Filtr: created_at >= 2026-04-01, zákazníci s více než 1 zakázkou (v tomto období)
 * Dávky: 500 řádků
 *
 * Samostatný Apps Script projekt (ne slučovat s testdata1 – kolize názvů funkcí).
 */

var TARGET_SPREADSHEET_ID = '1I4vnQ-AqWlHGmylMDnDyK-TWIH3UCXwLcMXPoNCbXJs';
var SHEET_NAME = 'testdata2';
var BATCH_SIZE = 500;
var MAX_MS_PER_RUN = 4.5 * 60 * 1000;
var MIN_CREATED_DATE = '2026-04-01';

var PROP_OFFSET = 'TESTDATA2_EXPORT_OFFSET';
var PROP_DONE = 'TESTDATA2_EXPORT_DONE';
...
function testDbConnectionTestdata2() {
  testDbConnection_(SHEET_NAME);
}

function exportTestdata2OneBatch() {
...
function exportTestdata2ToSheet() {
  resetExportProgressTestdata2();
  runExportBatches_(true);
}

function exportTestdata2Continue() {
  var props = PropertiesService.getScriptProperties();
  if (props.getProperty(PROP_DONE) === '1') {
    getSpreadsheet_().toast('Export už je hotový.', SHEET_NAME, 5);
    return;
...
function exportTestdata2Reset() {
  resetExportProgressTestdata2();
  getOrCreateSheet_().clear();
  getSpreadsheet_().toast('Reset OK', SHEET_NAME, 3);
}

function resetExportProgressTestdata2() {
  var props = PropertiesService.getScriptProperties();
...
function fetchBatchFromDb_(connStr, limit, offset) {
  ...
  var sql = buildTestdata2Sql(limit, offset);
  var rows = [];
...
function buildTestdata2Sql(limit, offset) {
  return (
    buildOrdersBaseCte_() +
...
function getConnStr_() {
  ...
  if (!connStr) {
    throw new Error('Chybí ERP_DB_CONNECTION_STRING. Spusť setConfigTestdata2().');
  }
...
function normalizeConnStr_(connStr) {
  ...
  if (!raw) {
    throw new Error('Connection string je prázdný (ERP_CONNECTION_STRING_FOR_SETUP).');
  }
  ...
  if (!/^postgres(?:ql)?:\/\//i.test(raw)) {
    throw new Error('Musí začínat postgres://');
  }
  if (/USER:PASSWORD@HOST/i.test(raw)) {
    throw new Error('Stále placeholder v ERP_CONNECTION_STRING_FOR_SETUP.');
  }
  return raw;
}
